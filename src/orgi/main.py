...
def sort_home(
    ...
    try:
        ...
        # Confirm with user
        confirm = typer.confirm(f"Do you want to organize these files into your Home directory?")
        if not confirm:
        ...
        typer.secho(f"✅ Organized {moved_count} files into your Home directory!", fg=typer.colors.GREEN)
        if skipped_count > 0:
...
def sort_here(
    ...
    try:
        ...
        if is_date_like_dir(src.name):
            typer.echo(f"The directory '{src.name}' appears to be a date-formatted directory. Skipping.")
            raise typer.Exit()
